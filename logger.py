...
def get_image_type(log_id, folder_name):
    ...
    if not path.is_dir():
        return None
    file_name = next(
        (f for f in os.listdir(path) if re.match("{}.(?:jpg|jpeg|png|tiff|webp)$".format(log_id), f)), None
    )
    if not file_name:
        return None
            ...
            for log in logs:
                if "matches" in log:
                    eel.updateLogDataById(
                        log["id"], {"matches": log["matches"], "autoMatch": True, "isMatched": False}
                    )()
    return
...
def update_log_text(log_id, folder_name, text):
    ...
    if Path(filename).is_file():
        temp_filename = "{}/temp.txt".format(TEXT_LOG_PATH)
        with codecs.open(filename, "r", encoding="utf-8") as fi, codecs.open(
...
def get_audio_file_name(log_id, folder_name):
    ...
    file_name = next(
        (
            ...
            if re.match("{}.(?:wav|mp3|mp4|ogg|wma|aac|aiff|flv|m4a|flac)$".format(log_id), f)
        ),
        None,
    )
