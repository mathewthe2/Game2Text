...
def attach_process(pids):
    textractor_thread = threading.Thread(
        target=start_textractor,
        args=[
            pids,
        ],
    )
...
def open_new_window(html_file, height=900, width=600):
    eel.start(
        ...
        mode=get_default_browser_name()
        if r_config(APP_CONFIG, "browser") == "default"
        else r_config(APP_CONFIG, "browser"),
...
def run_eel():
    ...
    browser_mode = (
        get_default_browser_name() if r_config(APP_CONFIG, "browser") == "default" else r_config(APP_CONFIG, "browser")
    )
